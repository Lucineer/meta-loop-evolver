# CLAUDE.md — Meta Loop Evolver

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Meta Loop Evolver
- **Role**: Meta Loop Evolver
- **URL**: https://meta-loop-evolver.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/meta-loop-evolver
- **Branch**: master
...
### Deploy
```bash
cd /tmp/meta-loop-evolver && wrangler deploy
```

### Health Check
```bash
curl -s https://meta-loop-evolver.casey-digennaro.workers.dev/health
curl -s https://meta-loop-evolver.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/experiment | API endpoint |
| /api/experiments | API endpoint |
| /api/propose | API endpoint |
| /api/rules | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=meta-loop-evolver
...
- **vessel.json capabilities**: rule-evolution, orchestration-improvement
